...
func main() {
	...
	var wordsFromStdin []string
	for scanner.Scan() {
    line := scanner.Text()
    fields := strings.Fields(line)
    wordsFromStdin = append(wordsFromStdin, fields...)
	}
	...
	mySlice := append(wordsFromCL, wordsFromStdin...)
	...
	var w1 string
	fmt.Scan(&w1)
	mySlice = append(mySlice[:2], append([]string{w1}, mySlice[2:]...)...)
	...
	var w2 string
	fmt.Scan(&w2)
	mySlice = append(mySlice, w2)
	fmt.Println("10.", mySlice)

	// 11. estende la slice con una nuova slice di lunghezza 2 (di stringhe vuote)
	mySlice = append(mySlice, make([]string, 2)...)
	fmt.Println("11.", mySlice)

	// 12. inserisce alla posizione 3 una nuova slice di lunghezza 3 (di stringhe vuote)
	empty3 := make([]string, 3)
	mySlice = append(mySlice[:3], append(empty3, mySlice[3:]...)...)
